...
tm_shape(test.map3) +
  tm_borders() 
dist.points <- SpatialPoints(dist.combo[, 2:3])
...
test5 <- test4 %>% 
  group_by(CT2010) %>% 
  summarize(avg_to_ada_stat = mean(any.vs.ada.dist))
test6 <- test4 %>% 
  group_by(NTAName) %>% 
  summarize(avg_to_ada_stat = mean(any.vs.ada.dist)) %>% 
...
tm_shape(merge.to.map2) +
  tm_fill("avg_to_ada_stat", style = "fixed", breaks = c(0, 0.25, 0.5, 0.75, 1.25, 2.5, 5), palette = inferno(n = 6))
merge.to.map.neigh <- merge(test.map3, test6, by.x = "NTAName", by.y = "NTAName")
tm_shape(merge.to.map.neigh) +
  tm_borders()+
  tm_fill("avg_to_ada_stat", style = "fixed", breaks = c(0.25, 0.5, 0.75, 1, 2, 3, 4), palette = viridis(n = 6))
