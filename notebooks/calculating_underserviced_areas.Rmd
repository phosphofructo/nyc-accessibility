...
subloc_bor <- subloc_raw %>% 
  # select(line:station_longitude) %>% 
  # distinct() %>% 
  mutate(latitude = round(station_latitude, 2),
         longitude = round(station_longitude, 2),
...
# odd <- subloc_bor %>% 
#  group_by(line, station_name, station_latitude, station_longitude) %>% 
#  count() %>% 
#  filter(n > 1)
#test <- subloc_bor %>% 
#  filter(station_name %in% odd$station_name & line %in% odd$line & station_latitude %in% odd$station_latitude & station_longitude %in% odd$station_longitude)
```
...
ggmap(nyc_map) +
  ...
  facet_wrap(~borough)
# there's a couple of wrong assignments - will worry about fixing those later
```
...
```{r ada_check}
head(subloc_bor$ada)
sub.loc.ada.sum <- subloc_bor %>% 
  group_by(station_location) %>%  # seems to be the best unique key for stations in the dataset
  mutate(ada_sum = sum(ada)) %>%  # which stations are ada accessible? 
  select(division:entrance_type, ada, station_location, ada_sum) %>% 
  distinct()
# Entrance types:
...
sub.loc.ada.sum %>% 
  ungroup() %>% 
  arrange(desc(ada_sum)) %>% 
  select(division:station_name, entrance_type:ada, ada_sum)
# stations with entrance.type == Stair are not ADA accessible
sub.loc.ada.sum %>% 
  ungroup() %>% 
  arrange(desc(station_name)) %>% 
  select(division:station_name, entrance_type:ada, ada_sum)
```
